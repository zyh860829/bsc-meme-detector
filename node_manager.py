...
class NodeManager:
    ...
    async def make_http_request(self, method, *args, **kwargs):
        ...
        if self._is_daily_limit_reached():
            self.logger.debug(f"⏭️ 达到每日限制，跳过HTTP请求: {method}")
            raise ConnectionError("达到每日扫描限制，跳过节点请求")
    ...
    async def _test_websocket_connection(self, ws_url: str) -> bool:
        ...
        # 🎯 新增：限制状态检查
        if self._is_daily_limit_reached():
            self.logger.debug(f"⏭️ 达到每日限制，跳过WebSocket测试: {ws_url}")
            return False
    ...
    async def get_current_websocket_url(self) -> str:
        """🎯 修改：添加限制检查的WebSocket URL获取"""
        # 🎯 新增：限制状态检查
        if self._is_daily_limit_reached():
            self.logger.debug("⏭️ 达到每日限制，跳过WebSocket连接")
            raise ConnectionError("达到每日扫描限制，跳过WebSocket连接")
    ...
    async def _auto_reconnect(self):
        ...
        while True:
            try:
                # 🎯 新增：限制状态下跳过重连
                if self._is_daily_limit_reached():
                    await asyncio.sleep(30)  # 限制状态下等待更长时间
                    continue
    ...
    async def check_node_health(self):
        ...
        # 🎯 新增：限制状态检查
        if self._is_daily_limit_reached():
            self.logger.debug("⏭️ 达到每日限制，跳过节点健康检查")
            return
