...
class CacheManager:
    ...
    def set(self, category, identifier, data, ttl=None):
        """设置缓存 - ✅ 修复：使用JSON序列化替代pickle"""
        if not self.redis_client:
            return False
            
        try:
            key = self.get_key(category, identifier)
            # ✅ 修复：使用JSON序列化，避免pickle安全风险
            serialized_data = json.dumps(data, default=str)
            if ttl:
                self.redis_client.setex(key, timedelta(seconds=ttl), serialized_data)
            else:
