...
class MemeTokenDetector:
    ...
    async def initialize(self):
        ...
        # 检查必要环境变量 - 修改：不再检查BSC_NODES，因为现在已硬编码
        if not self.config.DINGTALK_WEBHOOK:
            self.logger.error("缺少必要的环境变量配置: DINGTALK_WEBHOOK")
            self.logger.error(f"DINGTALK_WEBHOOK 为空: {self.config.DINGTALK_WEBHOOK is None}")
            return False

        # 检查硬编码节点是否可用
        if not self.config.BSC_NODES or len(self.config.BSC_NODES) == 0:
            self.logger.error("BSC节点配置为空")
            return False

        self.logger.info(f"已配置 {len(self.config.BSC_NODES)} 个BSC节点")
        ...
        # 👇 新增：启动 NodeManager（初始化 Redis、自动重连任务）
    ...
    async def start_http_server(self):
        ...
        # 绑定到所有接口的指定端口（优先使用环境变量PORT，否则用8080）
    ...
    async def shutdown(self):
        ...
        if self.event_listener:
            await self.event_listener.stop_listening()  # 👈 改这里：主动停止扫描

        # 👇 新增：关闭 NodeManager（释放 Redis 连接、取消任务）
    ...
    def signal_handler(self, signum, frame):
        ...
        # 使用 create_task 确保 shutdown 被调用
